...
class KisumuMpesaService implements CountyPaymentService {
  ...
  Future<Map<String, dynamic>> initiatePayment({
    ...
    try {
      // Format phone for Kisumu (remove leading 0, add 254)
      final formattedPhone = _formatKisPhone(phone);

      // Calculate litres based on Kisumu water rate
      final litres = county.calculateLitres(amount);

      // Generate transaction reference
      final transactionRef =
          'KIS${DateTime.now().millisecondsSinceEpoch}${meterNumber.substring(meterNumber.length - 4)}';

      // Call Kisumu County M-Pesa API
      final response = await http.post(
        Uri.parse('https://api.kisumu.go.ke/mpesa/v1/stkpush'),
        headers: {
        ...
        body: jsonEncode({
          ...
          'AccountReference': 'KISUMUWATER$meterNumber',
          'TransactionDesc': 'Kisumu Water Payment',
          'CountyMetadata': {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Kisumu M-Pesa API error: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Kisumu payment error: $e');
    }
  }
